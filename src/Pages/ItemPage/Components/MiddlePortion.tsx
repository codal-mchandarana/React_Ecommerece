...
const MiddlePortion: React.FC = (): JSX.Element => {
  ...
  const [edit, isEdit] = useState<edit>({
    price: "0",
    rating: "0",
    brand: "0",
    last: 0,
  ...
  const fn = async () => {
    setLoading(true);
    let newData = await fetchItemsAWS(0);
    setLoading(false);
    setData(newData);
  };
  ...
  const sortDataPrice = (type: number, vari: number, arr1: ProductType[]) => {
    ...
    if (vari === 1) arr = data1;
    else if (vari === 2) arr = arr1;

    arr.sort((item1, item2) => {
      if (type === 1) return parseInt(item1.price) - parseInt(item2.price);
      else return parseInt(item2.price) - parseInt(item1.price);
    });
    return arr;
  };
  ...
  const sortDataRating = (type: number, vari: number, arr1: ProductType[]) => {
    ...
    arr.sort((item1, item2) => {
      ...
      else return parseFloat(item2.rating) - parseFloat(item1.rating);
    });
    return arr;
  };
  ...
  const handleChange = (event: React.ChangeEvent<HTMLSelectElement>): void => {
    ...
    if (name === "price") {
      ...
      if (value === "No Choice") {
        ...
        if (edit.last === 1) arr = changePrice(edit.price, 2, arr);
        else if (edit.last === 2) arr = changeRating(edit.rating, 2, arr);
        setData((prev) => [...arr]);
      }
    }
  };
  ...
  const handleChangeInput = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value === "") {
      ...
      if (edit.last === 1) {
        ...
        if (edit.last === 3) arr = brandFiltering(edit.brand);
      }

      setData((prev) => [...arr]);
    } else arr = inputChanging(value.toLowerCase());
    ...
    <>
      ...
      <div className="container">
        ...
        <Pagination
          ...
          page={page}
          setPage={setPage}
          totalPage={Math.ceil(data1.length / 10)}
        />
      </div>
    </>
  );
